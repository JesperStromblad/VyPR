...
def consumption_thread_function(verification_obj):
    ...
    while continue_monitoring:
        ...
        # For getting the transaction time which is the start of the test suite i.e., first test class that is executed
        if top_pair[0] == "test_transaction":
            transaction = top_pair[1]
            vypr_output("Transaction time g: %s" % str(transaction))
            continue
        ...
        if first_element == "function":
            ...
            ## Ask JOSH why we are passing a list of hashes, why not single hash
            ...
            # Question for Josh: Property hash is a list, there is a single property hash on index 4. Why arnt we using that
            ...
            if instrument_type == 'function':
                ...
                if top_pair[2] == 'end':
                    ...
                    # If we detected a test framework which means that we are doing testing, then transaction time is the start of test suite
                    if 'yes' in TEST_FRAMEWORK :
                        transaction_time = transaction
                    else:
                        transaction_time = top_pair[4]
...
class Verification(object):
    ...
    def __init__(self):
        ...
        global VERDICT_SERVER_URL, VYPR_OUTPUT_VERBOSE, PROJECT_ROOT, VYPR_MODULE, TEST_FRAMEWORK
